...
public class KindEditorUploadController {
    ...
    @ResponseBody
    public Map<String, Object> fileUpload(HttpServletRequest request, HttpServletResponse response, String  uid) throws ServletException, IOException,
            FileUploadException {
        ServletContext application = request.getSession().getServletContext();
        String savePath = diskPath;
        // 文件保存目录URL
        String saveUrl = urls;
        // 定义允许上传的文件扩展名
        ...
        while (item.hasNext()) {
            ...
            msg.put("originalName", file.getOriginalFilename());
            msg.put("fileName", newFileName);
            msg.put("fileSize", file.getSize());
    ...
    public void getDownload(String fileId, HttpServletRequest request, HttpServletResponse response) {
        TFileInfo tfile = tFileInfoMapper.selectById(fileId);
        String url = tfile.getFilePath();
        getDownloads(url,request, response);
    }
    @GetMapping(value="/filedownloads")
    public void getDownloads(String url, HttpServletRequest request, HttpServletResponse response) {
    ...
    public Map deletefile(String fileId, HttpServletRequest request, HttpServletResponse response) {
        Map<String ,Object> map =new HashMap();
        TFileInfo tfile = tFileInfoMapper.selectById(fileId);
        File file = new File(tfile.getFilePath());
        try {
            if(file.exists()){
                file.delete();
                tFileInfoMapper.deleteById(fileId);
            }
            map.put("Code","200");
